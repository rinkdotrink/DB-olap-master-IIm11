...
\item Mit einer zusätzlichen List-Partitionierung auf die drei Monate(K6) in jedem Quartal müsste die Suche nach einem konstanten Datumswert(Use Case 3) noch schneller ablaufen, denn es sollte nur in der einen Partition gesucht werden. Interessant ist hierbei auch, wie die List-Partitionierung mit BETWEEN beim Use Case 2 umgeht.    
\item Mit einem Hash-Partitioning auf die 12 Monate(K7) würde es einen Vorteil bringen, wenn nur auf eine bzw.  drei Partitionen(ein Quartal)zugegriffen wird. Wenn jedoch alle Partitionen durchsucht werden, dann bringt es eher Nachteile. Die Frage ist wie reagiert Hash-Partitioning auf eine BETWEEN-Anweisung.  
\item Das Range-Partitioning auf die Quartale(K8) sollte eigentlich die Geschwindigkeit für den Use Case 2 mit der BETWEEN-Anweisung verkürzen. Denn bei dem verwendeten Range-Partitioning entspricht eine Partition jeweils einem Quartal und im Use Case 2 wird die Datum-Spalte nach Werten in einem Quartal gesucht.
\item Die Konfiguration 9 unterscheidet sich von der Konfiguration 10 dadurch, dass sie ein B-Tree-Index auf das Datum enthält. Bei beiden Konfigurationen werden insgesamt 16 Teilpartitionen auf das Datum erstellt. Es soll dadurch herausgefunden werden, ob die Teilpartitionierung bei diesen SELECT-Anweisungen die Ausführung beschleunigt. 
\end{enumerate}
